import {loadPlugins} from '@putout/engine-loader';
import {test} from 'supertape';
import {declare} from '@putout/plugin-declare';
import {splitPlugins} from './split-plugins.js';

test('anuma: splitPlugins', (t) => {
    const plugins = loadPlugins({
        rules: {
            'declare': 'on',
            'remove-debugger': 'on',
        },
        pluginNames: [
            'declare',
            'remove-debugger',
        ],
    });
    
    const [fast, rest] = splitPlugins(plugins);
    const restPlugin = {
        msg: '',
        options: {},
        plugin: {
            declare,
        },
        rule: 'declare',
    };
    
    const fastPlugin = {
        message: `Unexpected 'debugger' statement`,
        patterns: [
            ['debugger', ''],
        ],
        rule: 'remove-debugger',
    };
    
    const expected = [
        [fastPlugin],
        [restPlugin],
    ];
    
    t.deepEqual([fast, rest], expected);
    t.end();
});

test('anuma: splitPlugins: template variable: __a', (t) => {
    const plugins = loadPlugins({
        rules: {
            'tape': 'off',
            'tape/convert-ok-to-match': 'on',
            'remove-debugger': 'on',
        },
        pluginNames: ['tape', 'remove-debugger'],
    });
    
    const [result] = splitPlugins(plugins);
    const expected = [{
        message: `Use 't.match()' instead of 't.ok()'`,
        exclude: [
            't.ok(keys($A).includes($B))',
        ],
        patterns: [
            ['t.ok($A.includes($B))', 't.match($A, $B)'],
            ['t.ok($A.includes($B), $C)', 't.match($A, $B, $C)'],
            ['t.ok($A.test($B))', 't.match($B, $A)'],
            ['t.ok($A.test($B), $C)', 't.match($B, $A, $C)'],
        ],
        rule: 'tape/convert-ok-to-match',
    }, {
        message: `Unexpected 'debugger' statement`,
        patterns: [
            ['debugger', ''],
        ],
        rule: 'remove-debugger',
    }];
    
    t.deepEqual(result, expected);
    t.end();
});

test('anuma: splitPlugins: __args', (t) => {
    const plugins = loadPlugins({
        rules: {
            'conditions': 'off',
            'conditions/simplify': 'on',
        },
        pluginNames: ['conditions'],
    });
    
    const [result] = splitPlugins(plugins);
    const expected = [{
        message: 'Avoid useless conditions',
        patterns: [
            ['if ($A?.$B) {$A.$B($$$ARGS)}', '$A?.$B($$$ARGS)'],
            ['if ($A?.$B) $A.$B($$$ARGS)', '$A?.$B($$$ARGS)'],
            ['if ($A) $B; else $B', '$B'],
        ],
        rule: 'conditions/simplify',
    }];
    
    t.deepEqual(result, expected);
    t.end();
});

test('anuma: splitPlugins: no options', (t) => {
    const plugins = loadPlugins({
        rules: {
            'types': 'off',
            'types/apply-is-array': 'on',
        },
        pluginNames: ['types'],
    });
    
    const [result] = splitPlugins(plugins);
    const expected = [];
    
    t.deepEqual(result, expected);
    t.end();
});

test('anuma: splitPlugins: has exclude', (t) => {
    const plugins = loadPlugins({
        rules: {
            'promises': 'off',
            'promises/remove-useless-await': 'on',
        },
        pluginNames: ['promises'],
    });
    
    const [result] = splitPlugins(plugins);
    const expected = [];
    
    t.deepEqual(result, expected);
    t.end();
});

test('anuma: splitPlugins: replace is function', (t) => {
    const plugins = loadPlugins({
        pluginNames: [
            ['convert', {
                report: () => '',
                replace: () => ({
                    'const __a = __b': () => 'let __a = __b',
                }),
            }],
        ],
    });
    
    const [result] = splitPlugins(plugins);
    const expected = [];
    
    t.deepEqual(result, expected);
    t.end();
});

test('anuma: splitPlugins: report has args', (t) => {
    const plugins = loadPlugins({
        pluginNames: [
            ['convert', {
                report: (path) => `${path} hello`,
                replace: () => ({
                    'const __a = __b': 'let __a = __b',
                }),
            }],
        ],
    });
    
    const [result] = splitPlugins(plugins);
    const expected = [];
    
    t.deepEqual(result, expected);
    t.end();
});
